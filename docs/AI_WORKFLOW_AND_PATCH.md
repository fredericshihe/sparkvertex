# AI Workflow 与 Patch 完整链路详解

## 📋 目录
1. [系统架构概览](#系统架构概览)
2. [AI Workflow 完整链路](#ai-workflow-完整链路)
3. [Patch 系统完整链路](#patch-系统完整链路)
4. [🚀 高级优化路线图](#高级优化路线图-advanced-optimization-roadmap)
5. [高级 RAG 优化](#高级-rag-优化)
6. [自愈循环系统](#自愈循环系统)
7. [文件映射与导出函数](#文件映射与导出函数)

---

## 系统架构概览

### 整体架构全景图

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                           AI Workflow System (AI 工作流系统)                      │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│   用户请求 ────▶ ┌─────────────┐    ┌─────────────┐    ┌─────────────┐          │
│                  │ Text Cache  │───▶│   Intent    │───▶│ Search      │          │
│                  │ (语义缓存)  │    │ Classifier  │    │ Strategy    │          │
│                  └─────────────┘    └─────────────┘    └─────────────┘          │
│                         │                  │                  │                 │
│                         ▼                  ▼                  ▼                 │
│                  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐          │
│                  │ Cache Hit?  │    │ Local/LLM   │    │ GraphRAG    │          │
│                  │ (75% 相似)  │    │ Fallback    │    │ 依赖图扩展   │          │
│                  └─────────────┘    └─────────────┘    └─────────────┘          │
│                                                              │                  │
│                         ┌────────────────────────────────────┘                  │
│                         ▼                                                       │
│                  ┌─────────────────────────────────────────────────────┐        │
│                  │              Code RAG System (代码检索)               │        │
│                  │  • P3: 语义压缩预处理 (semanticPrecompress)            │        │
│                  │  • P2: 函数级别分块 (AST-based chunking)              │        │
│                  │  • 向量嵌入 + pgvector 搜索                            │        │
│                  │  • P2: GraphRAG 上下文扩展                             │        │
│                  └─────────────────────────────────────────────────────┘        │
│                                          │                                      │
│                                          ▼                                      │
│                  ┌─────────────────────────────────────────────────────┐        │
│                  │              AI Generation (LLM 生成)                 │        │
│                  │  • Gemini 2.5 Flash / Pro / 3 Pro                    │        │
│                  │  • 上下文: 意图 + 目标文件 + 参考代码                   │        │
│                  │  • 输出: SEARCH/REPLACE 补丁格式 (支持行号锚定)         │        │
│                  └─────────────────────────────────────────────────────┘        │
│                                          │                                      │
│                                          ▼                                      │
│                  ┌─────────────────────────────────────────────────────┐        │
│                  │              Patch System (补丁应用)                  │        │
│                  │  • P1: 行号锚定优先 (@L42-L58)                        │        │
│                  │  • AST Smart Patch                                    │        │
│                  │  • Token 精确/模糊匹配 (Levenshtein)                   │        │
│                  │  • 多层安全网: 语法/引用/长度验证                        │        │
│                  └─────────────────────────────────────────────────────┘        │
│                                          │                                      │
│                            ┌─────────────┴─────────────┐                        │
│                            ▼                           ▼                        │
│                     ┌───────────┐               ┌───────────┐                   │
│                     │  Success  │               │  Failure  │                   │
│                     └───────────┘               └───────────┘                   │
│                            │                           │                        │
│                            ▼                           ▼                        │
│                  ┌─────────────────┐         ┌─────────────────┐                │
│                  │ Reflection Check│         │ Self-Repair     │                │
│                  │ (反思代理)       │         │ (自愈循环)       │                │
│                  └─────────────────┘         └─────────────────┘                │
│                            │                           │                        │
│                            ▼                           ▼                        │
│                  ┌─────────────────┐         ┌─────────────────┐                │
│                  │ ✅ 完成         │         │ 重新生成补丁      │                │
│                  └─────────────────┘         │ (最多 2 次重试)   │                │
│                                              └─────────────────┘                │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### 核心设计原则

| 原则 | 说明 |
|------|------|
| **渐进式优化** | 优先使用快速方法 (本地缓存/规则)，失败时逐步尝试更复杂的策略 (LLM) |
| **安全第一** | 多层验证确保补丁不会破坏现有代码，任何异常立即回滚 |
| **智能缓存** | Text Cache + Semantic Cache 减少 LLM 调用，提升响应速度 |
| **事务性补丁** | 所有补丁在副本上执行，全部成功才提交 |
| **可观测性** | 详细日志记录，便于调试和优化 |

---

## AI Workflow 完整链路

### 🎯 整体架构图

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                              用户发送修改请求                                      │
│                     "把按钮颜色改成红色" / "修复登录Bug"                            │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Stage 1: Intent Classification (意图分类)                                       │
│  ├── 入口: app/api/generate/route.ts → classifyUserIntent()                     │
│  ├── 核心: lib/intent-classifier.ts → classifyIntentWithDeepSeek()              │
│  └── 输出: intent, targets[], referenceTargets[], reasoning                     │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Stage 2: Code RAG (代码检索增强生成)                                             │
│  ├── 入口: app/api/generate/route.ts → findRelevantCodeChunks()                 │
│  ├── 核心: lib/code-rag.ts → chunkCode() + embedding similarity                 │
│  └── 输出: relevantChunks[] (与用户请求最相关的代码片段)                           │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Stage 3: Smart Compression (智能压缩)                                           │
│  ├── 入口: app/api/generate/route.ts → compressCode()                           │
│  ├── 核心: lib/code-rag.ts → AST Skeletonization / Brutal Truncate              │
│  └── 输出: compressedCode (压缩后的代码上下文, ~60% 压缩率)                        │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Stage 4: LLM Generation (代码生成)                                              │
│  ├── 入口: Supabase Edge Function / 外部 LLM API                                │
│  ├── 输入: compressedCode + ragContext + userPrompt + System Prompt              │
│  └── 输出: <<<<SEARCH ... ==== REPLACE ... >>>> 格式的 Patch                     │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Stage 5: Patch Application (补丁应用)                                           │
│  ├── 入口: app/create/page.tsx → applyPatches()                                 │
│  ├── 核心: lib/patch.ts → Multi-strategy Token/AST Matching                     │
│  └── 输出: 修改后的完整源代码                                                      │
└─────────────────────────────────────────────────────────────────────────────────┘
```

---

### Stage 1: Intent Classification (意图分类)

**文件位置**: `lib/intent-classifier.ts`

**核心函数**: `classifyUserIntent()` → `classifyIntentWithDeepSeek()`

**🆕 Text Cache 集成 (2025-01-15)**

在调用 LLM 之前，首先查询 Text Cache：

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Intent Classification Pipeline                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   用户输入: "把按钮颜色改成红色"                                       │
│                 │                                                   │
│                 ▼                                                   │
│   ┌─────────────────────────────┐                                   │
│   │  1. Text Cache 查询         │   <── 🆕 语义缓存                  │
│   │  n-gram Jaccard ≥ 0.75?     │                                   │
│   └─────────────────────────────┘                                   │
│          │ HIT              │ MISS                                  │
│          ▼                  ▼                                       │
│    ┌───────────┐   ┌─────────────────────────┐                      │
│    │ 返回缓存   │   │  2. 本地关键词匹配       │                      │
│    │ (<1ms)    │   │  INTENT_KEYWORDS Map    │                      │
│    └───────────┘   └─────────────────────────┘                      │
│                         │                                           │
│                         ▼                                           │
│              ┌─────────────────────┐                                │
│              │ 置信度 > 0.7?        │                                │
│              └─────────────────────┘                                │
│                  │ YES          │ NO                                │
│                  ▼              ▼                                   │
│            ┌───────────┐  ┌─────────────────────┐                   │
│            │ 直接返回   │  │ 3. DeepSeek API     │                   │
│            │ (快速路径) │  │ (增强分类)           │                   │
│            └───────────┘  └─────────────────────┘                   │
│                                │                                    │
│                                ▼                                    │
│                    ┌─────────────────────────┐                      │
│                    │  4. 提取目标文件名       │                      │
│                    │  extractFileNamesFromText│                     │
│                    │  PascalCase 匹配         │                      │
│                    └─────────────────────────┘                      │
│                                │                                    │
│                                ▼                                    │
│                    ┌─────────────────────────┐                      │
│                    │  5. 存入 Text Cache     │   <── 🆕              │
│                    │  (TTL: 30min)           │                      │
│                    └─────────────────────────┘                      │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

**流程**:
```
1. 接收用户 Prompt + 代码文件摘要 (fileSummaries)
                    │
                    ▼
2. 生成文件摘要 (generateFileSummary)
   - 提取每个组件的 useState, useEffect, handlers, 依赖关系
   - 示例: "MapScreen: uses [position, exploredTiles], effects: [player movement], handlers: [handleMove]"
                    │
                    ▼
3. 构建 DeepSeek Prompt
   - 角色: Senior Software Architect
   - 任务: 分析 files_to_edit (需要修改) vs files_to_read (只读参考)
   - 输出格式:
     {
       "reasoning": "用户要求...",
       "intent": "UI_MODIFICATION | LOGIC_FIX | NEW_FEATURE | ...",
       "files_to_edit": ["MapScreen", "BattleScene"],
       "files_to_read": ["CONSTANTS", "Types"]
     }
                    │
                    ▼
4. 调用 Supabase Edge Function → DeepSeek API
   - 超时: 45s
   - 温度: 0.3 (偏保守)
   - Fallback: Gemini 2.5 Flash → 本地规则
                    │
                    ▼
5. 解析响应
   - 提取 JSON (支持 Markdown 代码块包裹)
   - 验证 intent 是否为有效枚举值
   - 从 reasoning 中提取遗漏的文件名 (Safety Net)
```

**意图类型 (UserIntent Enum)**:
| Intent | 描述 | 压缩阈值 |
|--------|------|----------|
| `UI_MODIFICATION` | 颜色、样式、布局 | 8 行 (最激进) |
| `LOGIC_FIX` | Bug 修复、数据流 | 12 行 |
| `NEW_FEATURE` | 新增页面/组件 | 15 行 (最保守) |
| `DATA_OPERATION` | 数据库、API | 12 行 |
| `REFACTOR` | 代码重构 | 10 行 |
| `PERFORMANCE` | 性能优化 | 12 行 |
| `GLOBAL_REVIEW` | 全局检查 | 特殊处理 |

---

### Stage 2: Code RAG (代码检索增强)

**文件位置**: `lib/code-rag.ts`

**核心函数**: `findRelevantCodeChunks()`

**流程**:
```
1. 🆕 语义压缩预处理 (P3 优化: semanticPrecompress)
   - 删除单行注释 (保留 JSDoc)
   - 合并连续空行 (最多 1 个)
   - 删除 console.log/debug 语句
   - 截断超长字符串 (>200 字符)
   - 效果: 节省 15-25% Token
                    │
                    ▼
2. 🆕 函数级分块 (P2 优化: AST-based chunking)
   - 使用 Babel Parser 解析 AST
   - 按 React 组件/函数/常量边界分块
   - 保留完整函数上下文
   - 每个 Chunk: { id, content, type, dependencies }
                    │
                    ▼
3. 向量嵌入 (Embedding)
   - Gemini gemini-embedding-001 模型
   - 768 维向量
   - 缓存优化: 相同代码不重复嵌入
                    │
                    ▼
4. 相似度搜索 (Cosine Similarity)
   - pgvector 向量数据库
   - 计算 Prompt 与每个 Chunk 的相似度
   - 示例: MapScreen=0.715, BattleScene=0.742
                    │
                    ▼
5. 🆕 GraphRAG 上下文扩展 (P2 优化)
   - 构建依赖图 (buildDependencyGraph)
   - 向下扩展: 目标依赖的模块 → readOnly 上下文
   - 向上扩展: 依赖目标的模块 → 可能需要修改
   - 限制: maxDepth=2, maxNodes=15
```

**🆕 P2 优化: GraphRAG 依赖图**

```typescript
interface DependencyGraph {
    dependencies: Map<string, string[]>;  // 节点 → 依赖列表
    dependents: Map<string, string[]>;    // 节点 → 被依赖列表
    nodes: string[];
}

// 构建依赖图
function buildDependencyGraph(chunks: CodeChunk[]): DependencyGraph {
    // 1. 分析每个 Chunk 的 import 和 JSX 使用
    // 2. 建立双向索引
    // 3. 返回完整依赖图
}

// 智能上下文扩展
function expandContextByGraph(targetIds, graph, options): { edit: string[]; read: string[] } {
    // BFS 遍历依赖图
    // 深度 1 的父节点 → edit (可能需要修改)
    // 深度 1 的子节点 → read (仅需参考)
    // 深度 > 1 → read
}
```

**示例:**
```
目标: MapScreen

依赖图扩展结果:
├── edit (需要修改):
│   ├── MapScreen (目标)
│   └── GameScreen (使用 MapScreen, 可能需要调整)
│
└── read (仅参考):
    ├── TILE_CLASSES (MapScreen 依赖)
    ├── VISIBILITY_RADIUS (MapScreen 依赖)
    └── App (根组件)
```

**🆕 P3 优化: 语义压缩预处理**

```typescript
function semanticPrecompress(code: string, options: {
    removeComments?: boolean;       // 删除注释 (保留 JSDoc)
    collapseBlankLines?: boolean;   // 合并空行
    removeConsoleLogs?: boolean;    // 删除 console.log
    truncateLongStrings?: boolean;  // 截断长字符串
    maxStringLength?: number;       // 默认 200
}): { code: string; stats: CompressionStats }

// 示例效果:
// 原始: 12,500 字符
// 压缩: 9,800 字符
// 节省: 21.6%
```

**🆕 P0 优化: STRICT MODE (信任 DeepSeek)**
```typescript
// 当 DeepSeek 提供了 files_to_edit/files_to_read 时
// 完全信任 DeepSeek，禁用基于字符串的模糊匹配
// 防止 "screen" 匹配 "LaunchScreen", "DexScreen" 等噪音

const TRUST_DEEPSEEK_ONLY = explicitTargets.length > 0 || referenceTargets.length > 0;

if (TRUST_DEEPSEEK_ONLY) {
    // 只允许精确匹配完整组件名
    // 禁用中文关键词模糊匹配
}
```

**函数名→父文件解析** (新增修复):
```typescript
// 如果 DeepSeek 返回函数名 "handleMove" 而不是组件名 "MapScreen"
// 系统会自动解析: handleMove → 定义在 MapScreen → 保留 MapScreen 完整代码
```

---

### Stage 3: Smart Compression (智能压缩)

**文件位置**: `lib/code-rag.ts`

**核心函数**: `compressCode()` → `skeletonizeCode()`

**压缩策略分层**:

```
┌─────────────────────────────────────────────────────────────────┐
│  Layer 1: Primary Targets (Explicit Files to Edit)              │
│  策略: 📝 FULL CODE - 完整保留，不做任何压缩                      │
│  示例: MapScreen (用户指定要修改的组件)                          │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│  Layer 2: Context References (files_to_read)                    │
│  策略: 📖 AST Skeleton - 保留函数签名，隐藏函数体                 │
│  示例:                                                          │
│    // Before:                                                   │
│    function BattleScene({ player, enemy }) {                    │
│      const [hp, setHp] = useState(100);                         │
│      // ... 260 lines ...                                       │
│      return <div>...</div>                                      │
│    }                                                            │
│    // After:                                                    │
│    function BattleScene({ player, enemy }) {                    │
│      /* Body hidden: 45 statements */                           │
│    }                                                            │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│  🆕 Layer 3: Data/Config Files (Pure Constants)                 │
│  策略: 📊 Data Skeletonization - 保留所有 KEY，截断长 VALUE      │
│  🆕 P0 优化: 不再使用 Brutal Truncate！                          │
│  示例:                                                          │
│    // Before: VISIBILITY_RADIUS (131 lines)                     │
│    export const VISIBILITY_RADIUS = 3;                          │
│    export const MAP_WIDTH = 20;                                 │
│    export const LARGE_ARRAY = [/* 100 items */];                │
│                                                                 │
│    // After: 保留所有导出的 KEY 名称！                            │
│    export const VISIBILITY_RADIUS = 3;                          │
│    export const MAP_WIDTH = 20;                                 │
│    export const LARGE_ARRAY = [ /* 100 items - see source */ ]; │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│  Layer 4: Irrelevant Modules (Non-data, Non-target)             │
│  策略: 🗜️ Brutal Truncate - 只保留前 10-15 行                    │
│  适用: 非数据文件的不相关代码 (如 ShopScreen 在修改 MapScreen 时)│
└─────────────────────────────────────────────────────────────────┘
```

**🆕 P0 优化: Data Skeletonization (数据骨架化)**
```typescript
// 检测纯数据文件 (只有 const exports, 没有函数)
// 保留所有导出的变量名，只截断长值

function dataSkeletonize(code: string, chunkId: string) {
    // 1. 检测: 有 exports，无 functions
    const isPureDataFile = /export\s+const/.test(code) && 
                          !/function\s+\w+/.test(code);
    
    // 2. 遍历所有 const 声明
    // 3. 短值 (<300 chars) → 完整保留
    // 4. 长值 (数组/对象) → 保留结构 + "N items truncated"
}
```

**AST Skeletonization 算法**:
```typescript
// 使用 Babel Parser 解析 → traverse → 替换函数体
traverse(ast, {
    "FunctionDeclaration|ArrowFunctionExpression"(path) {
        if (bodySize > 100 || statements > 3) {
            path.get("body").replaceWith(
                t.blockStatement([]) // 空函数体
            );
            // 添加注释: /* Body hidden: 45 statements */
        }
    }
});
```

---

### 🆕 自适应压缩策略 (Adaptive Compression Strategy)

针对"饱和式上下文"场景（如多组件互动 UI 优化），系统会智能判断是否需要全量保留代码。

**决策流程图:**
```
┌─────────────────────────────────────────────────────────────────────┐
│                    Adaptive Compression Strategy                    │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  文件大小 < 500 行?                                                  │
│     │ YES                    │ NO                                   │
│     ▼                        ▼                                      │
│  ┌─────────────────┐   ┌─────────────────────────────────────────┐  │
│  │ 📝 全量保留      │   │ 🔪 Program Slicing (程序切片)            │  │
│  │ Full Code       │   │                                         │  │
│  │ 无压缩          │   │  1. buildDataFlowGraph() 构建数据流图    │  │
│  └─────────────────┘   │  2. 提取目标函数 + 依赖链                │  │
│                        │  3. computeProgramSlice('backward')     │  │
│                        └─────────────────────────────────────────┘  │
│                                         │                           │
│                                         ▼                           │
│                        ┌─────────────────────────────────────────┐  │
│                        │ 切片后 < 500 行?                         │  │
│                        │   │ YES             │ NO                │  │
│                        │   ▼                 ▼                   │  │
│                        │ ✅ 使用切片      🗜️ AST Skeleton        │  │
│                        │   (精确上下文)     (保留签名+注释)       │  │
│                        └─────────────────────────────────────────┘  │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

**适用场景对比:**

| 场景 | 压缩策略 | 原因 |
|------|----------|------|
| 修复单个 Bug | Program Slicing | 只需看相关数据流 |
| 改按钮颜色 | 激进压缩 (8行) | 只需看样式定义 |
| **多组件互动 UI** | **全量保留** | 需要看组件间通信 |
| 大文件 (>500行) | Slicing → Skeleton | 防止 Token 爆炸 |

**代码实现:**
```typescript
function selectCompressionStrategy(
    chunk: CodeChunk, 
    intent: UserIntent,
    isMultiComponentInteraction: boolean
): CompressionLevel {
    const lineCount = chunk.content.split('\n').length;
    
    // 场景 1: 多组件互动 (如 UI 优化) → 饱和式保留
    if (isMultiComponentInteraction && lineCount < 500) {
        console.log(`[Compression] 📝 Full code for ${chunk.id} (multi-component interaction)`);
        return 'FULL';
    }
    
    // 场景 2: 小文件 → 直接全量保留
    if (lineCount < 500) {
        return 'FULL';
    }
    
    // 场景 3: 大文件 → 尝试 Program Slicing
    const slice = computeProgramSlice(chunk.content, targetFunction, 'backward');
    if (slice && slice.code.split('\n').length < 500) {
        console.log(`[Compression] 🔪 Sliced ${chunk.id}: ${lineCount} → ${slice.code.split('\n').length} lines`);
        return 'SLICED';
    }
    
    // 场景 4: 切片后仍然太大 → AST Skeleton
    console.log(`[Compression] 🦴 Skeleton for ${chunk.id} (${lineCount} lines too large)`);
    return 'SKELETON';
}
```

**效果示例:**
```
原始 App.js: 2000 行
├── 目标函数: handleAnswer
├── Backward Slice: 提取相关代码 150 行
├── 压缩比: 92.5%
└── 保留: handleAnswer + 依赖的 state/constants
```

**日志识别:**
```
[Compression] 📝 Full code for QuestionCard (multi-component interaction)
[Compression] 📝 Full code for ProgressBar (multi-component interaction)
[Compression] 🔪 Sliced App: 2000 → 150 lines
```

---

## 🚀 高级优化路线图 (Advanced Optimization Roadmap)

> 当前瓶颈：**Stage 3 (Smart Compression)** 和 **Stage 4 (LLM Generation)** 之间的 Token 消耗。
> 
> 目标：在**不降低质量**的前提下，大幅减少 Token 消耗。

### 优化全景表

| 优先级 | 优化手段 | 难度 | Token 节省潜力 | 核心价值 | 状态 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **P0** | **Prompt Caching (API级)** | 低 | **90% cost** | 几乎零成本实现巨大成本/速度优化 | ✅ 已实现 |
| **P1** | **Type Definitions (.d.ts)** | 中 | 30% | 针对参考文件的极大压缩，保留核心语义 | ✅ 已实现 |
| **P2** | **Program Slicing (切片)** | 高 | **50%+** | 真正的"智能"上下文，去除所有噪音 | ✅ 已实现 |
| **P3** | **GraphRAG Pruning** | 中 | 15-25% | 防止复杂项目中的上下文爆炸 | ✅ 已实现 |
| **P4** | **Unified Diff 输出** | 低 | 30% output | 减少输出 Token，提升生成速度 | ✅ 已实现 |

---

### P0: Prompt Caching (API级缓存) ⚡ ✅ 已实现

> **实现文件**: [lib/prompt-cache.ts](../lib/prompt-cache.ts)
> 
> 实现了三级缓存系统：L1 系统提示词缓存、L2 项目骨架缓存、L3 会话上下文缓存

**现状问题**：每次请求都发送完整的 System Prompt、项目结构等重复内容。

**优化方案**：利用 LLM 提供商（DeepSeek V3, Gemini 1.5, Anthropic）的 **Context Caching** 功能。

**分层 Prompt 架构:**
```
┌─────────────────────────────────────────────────────────────────────┐
│                    Layered Prompt Architecture                      │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Layer 1: Static (缓存 TTL: 长, 如 1 小时)                           │
│  ├── System Prompt (角色定义、输出格式规范)                           │
│  ├── 核心项目规范 (编码风格、命名约定)                                 │
│  └── 常用工具库定义 (React hooks, 通用类型)                          │
│                                                                     │
│  Layer 2: Project Structure (缓存 TTL: 中等, 如 10 分钟)             │
│  ├── 完整文件树 (目录结构)                                           │
│  ├── 组件依赖图摘要                                                  │
│  └── 全局类型定义 (types/*.ts)                                      │
│                                                                     │
│  Layer 3: Dynamic (不缓存, 每次请求动态生成)                          │
│  ├── 用户的具体请求                                                  │
│  ├── RAG 检索到的代码片段                                            │
│  └── 当前文件的压缩上下文                                             │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

**实现方式:**
```typescript
// 使用 Gemini Context Caching API
const cachedContent = await cacheManager.createCachedContent({
    model: 'gemini-1.5-pro',
    displayName: 'spark-vertex-system-prompt',
    contents: [
        { role: 'user', parts: [{ text: SYSTEM_PROMPT }] },
        { role: 'user', parts: [{ text: PROJECT_STRUCTURE }] }
    ],
    ttl: '3600s' // 1 小时
});

// 后续请求复用缓存
const response = await model.generateContent({
    cachedContent: cachedContent.name,
    contents: [dynamicUserRequest]
});
```

**预期收益:**
- 💰 计费 Token 减少 **70% - 90%**
- ⚡ 首字延迟 (TTFT) 大幅降低
- 🔄 无需改动现有逻辑

---

### P1: Type Definitions (.d.ts) 代替代码骨架 📝 ✅ 已实现

> **实现文件**: [lib/advanced-rag.ts](../lib/advanced-rag.ts) - `generateTypeDefinition()` 函数
> 
> **集成位置**: [lib/code-rag.ts](../lib/code-rag.ts) - `applyTrustModeCompression()` 函数
> 
> 对参考文件 (files_to_read) 自动生成类型定义，节省 30-50% Token

**现状问题**：对于 Reference Targets（参考文件），目前保留函数头和空函数体，仍占用较多 Token。

**优化方案**：对于非编辑目标（只读文件），生成 **TypeScript 声明文件风格的类型摘要**。

**对比示例:**
```typescript
// ❌ 当前: AST Skeleton (120 tokens)
function BattleScene({ player, enemy }: { player: Player; enemy: Enemy }) {
    /* Body hidden: 45 statements */
}

// ✅ 优化后: Type Definition (35 tokens)
declare function BattleScene(props: { player: Player; enemy: Enemy }): JSX.Element;

// ❌ 当前: React 组件骨架 (80 tokens)
const GameContext = createContext<GameState | null>(null);
export function GameProvider({ children }: { children: React.ReactNode }) {
    /* Body hidden: 12 statements */
}

// ✅ 优化后: 类型定义 (25 tokens)
declare const GameContext: React.Context<GameState | null>;
declare function GameProvider(props: { children: React.ReactNode }): JSX.Element;
```

**生成规则:**
```typescript
function generateTypeDefinition(chunk: CodeChunk): string {
    const ast = parse(chunk.content, BABEL_PARSER_CONFIG);
    const declarations: string[] = [];
    
    traverse(ast, {
        FunctionDeclaration(path) {
            const name = path.node.id?.name;
            const params = extractParamTypes(path.node.params);
            const returnType = inferReturnType(path.node);
            declarations.push(`declare function ${name}(${params}): ${returnType};`);
        },
        
        VariableDeclaration(path) {
            for (const decl of path.node.declarations) {
                if (t.isIdentifier(decl.id)) {
                    const type = inferType(decl.init);
                    declarations.push(`declare const ${decl.id.name}: ${type};`);
                }
            }
        },
        
        // React 组件 Props 特殊处理
        TSInterfaceDeclaration(path) {
            // 保留完整的 interface 定义
            declarations.push(generate(path.node).code);
        }
    });
    
    return declarations.join('\n');
}
```

**预期收益:**
- 📉 Token 减少 **30%**
- 🧠 保留足够的语义信息供 LLM 推理
- ✅ 类型安全，不易出错

---

### P2: Program Slicing (语义切片) 🔪

> ✅ **已在 `lib/advanced-rag.ts` 中实现**

**现状问题**：AST 骨架化保留文件结构，但如果一个文件有 50 个函数，只改其中 1 个，另外 49 个函数的"空壳"依然占用 Token。

**解决方案**：基于 **数据流 (Data Flow)** 和 **控制流 (Control Flow)** 分析的静态程序切片。

**工作原理:**
```
┌─────────────────────────────────────────────────────────────────────┐
│                    Program Slicing Pipeline                         │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  输入: 目标变量 `userBalance`                                        │
│                     │                                               │
│                     ▼                                               │
│  1. 构建数据流图 (buildDataFlowGraph)                                │
│     ├── 变量定义位置                                                 │
│     ├── 变量使用位置                                                 │
│     └── 变量间依赖关系                                               │
│                     │                                               │
│                     ▼                                               │
│  2. 计算 Backward Slice                                             │
│     └── 所有 **影响** `userBalance` 的语句                           │
│         ├── initialBalance (常量)                                   │
│         ├── calculateBonus() (函数调用)                              │
│         └── user.rewards (属性访问)                                  │
│                     │                                               │
│                     ▼                                               │
│  3. 计算 Forward Slice                                              │
│     └── 所有 **被 `userBalance` 影响** 的语句                        │
│         ├── displayBalance() (UI 渲染)                              │
│         ├── canPurchase() (逻辑判断)                                 │
│         └── saveToStorage() (副作用)                                │
│                     │                                               │
│                     ▼                                               │
│  4. 合并切片 + 添加上下文行                                           │
│     └── 输出: 精确的代码片段 (原始 2000 行 → 150 行)                  │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

**与 AST Skeleton 的对比:**
```
原始文件: PaymentService.ts (2000 行, 50 个函数)
目标: 修改 calculateDiscount()

┌─────────────────────────────────────────────────────────────────────┐
│  方案                    │ 保留内容                    │ Token 数   │
├─────────────────────────────────────────────────────────────────────┤
│  Full Code              │ 全部 2000 行                │ ~8000      │
│  AST Skeleton           │ 50 个函数签名 + 空函数体     │ ~2500      │
│  🆕 Program Slicing     │ 仅 3 个相关函数 + 依赖       │ ~800       │
└─────────────────────────────────────────────────────────────────────┘

Token 节省: 68% (vs Skeleton), 90% (vs Full)
```

**预期收益:**
- 📉 Token 减少 **40% - 60%**
- 🎯 "手术刀"级别的精准上下文
- 🧠 减少噪音，提升 LLM 聚焦度

---

### P3: GraphRAG Pruning (依赖图剪枝) ✂️ ✅ 已实现

> **实现文件**: [lib/advanced-rag.ts](../lib/advanced-rag.ts) - `computePageRank()`, `pruneGraphByPageRank()`
> 
> **集成位置**: [lib/code-rag.ts](../lib/code-rag.ts) - GraphRAG 扩展流程中自动启用
> 
> 对超过 15 个节点的依赖图自动进行 PageRank 剪枝

**现状问题**：GraphRAG 做"上下文扩展"时，可能引入过多噪音（不是所有依赖都同等重要）。

**优化方案**：引入 **PageRank / 中心度算法** 进行剪枝。

**剪枝策略:**
```typescript
interface PruningConfig {
    maxDistance: number;        // 最大依赖距离 (默认 2)
    minRelevanceScore: number;  // 最小相关性阈值 (默认 0.3)
    maxNodes: number;           // 最大节点数 (默认 15)
}

function pruneGraphRAG(
    graph: DependencyGraph,
    targetIds: string[],
    config: PruningConfig
): DependencyGraph {
    const relevanceScores = new Map<string, number>();
    
    // 1. 计算每个节点与目标的语义距离
    for (const nodeId of graph.nodes) {
        const distance = calculateGraphDistance(targetIds, nodeId, graph);
        const referenceCount = countReferences(nodeId, graph);
        
        // PageRank 风格的评分
        relevanceScores.set(nodeId, 
            (1 / (distance + 1)) * Math.log(referenceCount + 1)
        );
    }
    
    // 2. 区分强弱依赖
    const strongDeps = new Set<string>();  // import 语句
    const weakDeps = new Set<string>();    // useRef 内部引用等
    
    for (const [nodeId, deps] of graph.dependencies) {
        for (const dep of deps) {
            if (isImportDependency(nodeId, dep)) {
                strongDeps.add(dep);
            } else {
                weakDeps.add(dep);
            }
        }
    }
    
    // 3. 剪枝: 低相关性 + 弱依赖 → 丢弃或只保留名称
    const prunedNodes = graph.nodes.filter(nodeId => {
        const score = relevanceScores.get(nodeId) || 0;
        
        // 强依赖: 保留
        if (strongDeps.has(nodeId)) return true;
        
        // 目标节点: 保留
        if (targetIds.includes(nodeId)) return true;
        
        // 低相关性弱依赖: 剪枝
        if (score < config.minRelevanceScore && weakDeps.has(nodeId)) {
            console.log(`[GraphRAG] ✂️ Pruned: ${nodeId} (score: ${score.toFixed(2)})`);
            return false;
        }
        
        return true;
    });
    
    return { ...graph, nodes: prunedNodes };
}
```

**日志示例:**
```
[GraphRAG] 📊 原始依赖图: 25 节点
[GraphRAG] ✂️ Pruned: DebugUtils (score: 0.12)
[GraphRAG] ✂️ Pruned: TestHelpers (score: 0.08)
[GraphRAG] ✂️ Pruned: OldMigrations (score: 0.05)
[GraphRAG] 📊 剪枝后: 18 节点 (减少 28%)
```

**预期收益:**
- 📉 防止上下文随依赖深度指数级膨胀
- 🎯 保留高相关性依赖
- 🧹 去除调试工具、测试辅助等噪音

---

### P4: Unified Diff 输出格式 (可选) 📄 ✅ 已实现

> **实现文件**: [lib/advanced-rag.ts](../lib/advanced-rag.ts) - `generateUnifiedDiff()`, `applyUnifiedDiff()`
> 
> **导出位置**: [lib/code-rag.ts](../lib/code-rag.ts) - 导出供外部使用
> 
> 提供完整的 Unified Diff 生成和应用功能

**现状问题**：`SEARCH/REPLACE` 格式需要在 SEARCH 块中重复旧代码，增加输出 Token。

**优化方案**：对于简单修改，支持 **Unified Diff** 格式。

**格式对比:**
```diff
# ❌ 当前: SEARCH/REPLACE (60 output tokens)
<<<<SEARCH
const theme = {
    primary: '#3B82F6',
    secondary: '#10B981',
    background: '#F3F4F6'
};
====
const theme = {
    primary: '#EF4444',    // 改为红色
    secondary: '#10B981',
    background: '#F3F4F6'
};
>>>>

# ✅ 优化后: Unified Diff (25 output tokens)
@@ -1,5 +1,5 @@
 const theme = {
-    primary: '#3B82F6',
+    primary: '#EF4444',    // 改为红色
     secondary: '#10B981',
     background: '#F3F4F6'
 };
```

**混合模式策略:**
```typescript
type PatchFormat = 'search-replace' | 'unified-diff';

function selectPatchFormat(modification: Modification): PatchFormat {
    // 大段重构: 保持 SEARCH/REPLACE (稳健)
    if (modification.linesChanged > 10) {
        return 'search-replace';
    }
    
    // 多处分散修改: 保持 SEARCH/REPLACE
    if (modification.locations > 3) {
        return 'search-replace';
    }
    
    // 小修补 (改配置、改一行): 使用 Unified Diff
    if (modification.linesChanged <= 3 && modification.isSimpleChange) {
        return 'unified-diff';
    }
    
    return 'search-replace'; // 默认稳健模式
}
```

**风险与对策:**
- ⚠️ **LLM 容易算错行号** → 使用内容锚点验证 + 自动修正
- ⚠️ **格式切换增加复杂度** → 先在低风险场景试点

**预期收益:**
- 📉 输出 Token 减少 **30%**
- ⚡ 间接提升生成速度
- 🔧 适用于配置修改、样式调整等简单场景

---

### 实施路线图

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Implementation Timeline                          │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Phase 1 (Quick Wins) - 1-2 周                                      │
│  ├── ✅ P2: Program Slicing (已完成)                                │
│  ├── 🔜 P0: Prompt Caching 集成                                     │
│  │   └── 接入 Gemini/DeepSeek Context Cache API                    │
│  └── 🔜 P1: Type Definitions 生成器                                 │
│      └── 新增 generateTypeDefinition() 函数                         │
│                                                                     │
│  Phase 2 (Deep Optimization) - 3-4 周                               │
│  ├── 🔜 P3: GraphRAG Pruning                                        │
│  │   └── 集成 PageRank 评分 + 剪枝逻辑                               │
│  └── 🔜 P4: Unified Diff 支持 (可选)                                │
│      └── 新增 diff 解析器 + 混合模式选择                             │
│                                                                     │
│  预期综合效果:                                                       │
│  ├── Token 消耗: -60% ~ -80%                                        │
│  ├── API 成本: -70% ~ -90% (Prompt Caching)                        │
│  └── 响应速度: +30% ~ +50%                                          │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

### Stage 4: LLM Generation (代码生成)

**输入构建**:
```
System Prompt (lib/prompts.ts):
├── 角色定义: Expert React Developer
├── 输出格式: <<<<SEARCH ... ==== REPLACE ... >>>>
├── 规则:
│   ├── 不要假设代码存在，使用提供的上下文
│   ├── 每个 Patch 块要包含足够的上下文
│   └── 不要添加不存在的注释

**🧱 国内网络可达性（系统级 Prompt 约束）**

为避免“国内打不开/加载慢”的问题，系统 Prompt 必须遵循：

- **默认不依赖第三方图片/字体/图标库**：优先使用 CSS 渐变、内联 SVG、系统字体栈；不要在生成代码中引入外部字体服务或随机图片服务。
- **如必须使用脚本 CDN（仅限少数情况）**：优先选择国内普遍可用的公共 CDN；避免依赖海外/Cloudflare 体系的 CDN。

**推荐的国内公共 CDN（常见前端库）**

- Staticfile（七牛/国内常用镜像）：`https://cdn.staticfile.org/`
- BootCDN（国内常用镜像）：`https://cdn.bootcdn.net/ajax/libs/`

**推荐的国内“自建/对象存储 + CDN”方案（图片/文件/字体等静态资源）**

- 阿里云 OSS + CDN
- 腾讯云 COS + CDN
- 七牛云 Kodo + CDN
- 又拍云（UPYUN）
- 华为云 OBS + CDN
- 百度智能云 BOS + CDN

> 实践建议：对生产站点，优先“自托管（同域）/对象存储 + CDN”，不要在运行时从不确定的第三方服务拉取图片、字体或图标。

User Prompt:
├── compressedCode (压缩后的完整代码)
├── ragContext (RAG 选中的代码片段)
├── codeContext (优化提示)
└── 用户原始请求
```

**输出格式**:
```diff
<<<<SEARCH
const MapScreen = ({ player, onMove }) => {
    const [position, setPosition] = useState({ x: 5, y: 5 });
====
const MapScreen = ({ player, onMove }) => {
    const [position, setPosition] = useState({ x: 5, y: 5 });
    const [fogOfWar, setFogOfWar] = useState(true); // 新增
>>>>
```

---

## Patch 系统完整链路

### 🎯 整体架构图

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                           LLM 输出的 Patch 文本                                  │
│  <<<<SEARCH @L42-L58 ... ==== REPLACE ... >>>>  或  <<<<AST_REPLACE: Name>>>>   │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Step 0: Patch 格式检测                                                          │
│  ├── AST_REPLACE 格式 → 走 Explicit AST 路径                                    │
│  ├── @L[start]-L[end] 行号锚定 → 走 Line Anchored 路径 🆕                        │
│  └── SEARCH/REPLACE 格式 → 走 Token Matching 路径                               │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
              ┌────────────────────┬─────┴────────────────┬───────────────────────┐
              ▼                    ▼                      ▼                       │
┌─────────────────────┐  ┌─────────────────────┐  ┌─────────────────────────────┐│
│ Explicit AST Replace│  │ 🆕 Line Anchored    │  │  Token-based Matching       ││
│ (Scheme 2: Modular) │  │  (P1 优化)           │  │  (Scheme 1: Cursor-style)   ││
└─────────────────────┘  └─────────────────────┘  └─────────────────────────────┘│
              │                    │                      │                       │
              ▼                    ▼                      ▼                       │
┌─────────────────────┐  ┌─────────────────────┐  ┌─────────────────────────────┐│
│applyExplicitASTPatch│  │applyLineAnchoredPatch│ │  applyPatchesInternal()     ││
│ 直接定位变量/函数名   │  │ 行号范围 + 相似度验证 │  │  多策略匹配 (见下方详解)      ││
└─────────────────────┘  └─────────────────────┘  └─────────────────────────────┘│
                                         │                                        │
                                         ▼                                        │
┌─────────────────────────────────────────────────────────────────────────────────┘
│  Post-Validation (安全校验)                                                      │
│  ├── 长度检查: 结果不能太短 (<50% 原始) 或太长 (>300%)                            │
│  ├── 语法修复: validateAndFixSyntax() - 修复常见语法错误                         │
│  ├── 定义去重: dedupeTopLevelBindings() - 删除重复的变量/函数定义                 │
│  ├── 引用检测: detectUndefinedReferences() - 检测删除了但仍被引用的组件            │
│  └── 回滚机制: 任何校验失败 → 返回原始代码                                        │
└─────────────────────────────────────────────────────────────────────────────────┘
```

---

### 🆕 P1 优化: 行号锚定匹配 (Line Anchored Matching)

**补丁格式:**
```
<<<< SEARCH @L42-L58        ← 行号锚定标记
const gridColor = "blue";   ← 原始代码 (用于验证)
====
const gridColor = "red";    ← 新代码
>>>>
```

**工作原理:**
```typescript
// 解析行号范围
const extractLineNumbers = (fullMatch: string): { startLine?: number; endLine?: number } => {
    const lineMatch = fullMatch.match(/@L(\d+)(?:-L(\d+))?/);
    if (lineMatch) {
        return {
            startLine: parseInt(lineMatch[1], 10),  // 开始行 (1-based)
            endLine: lineMatch[2] ? parseInt(lineMatch[2], 10) : undefined
        };
    }
    return {};
};

// 行号锚定匹配
const applyLineAnchoredPatch = (source, searchBlock, replaceBlock, lineInfo) => {
    if (!lineInfo.startLine) return null;
    
    const sourceLines = source.split('\n');
    const searchLines = searchBlock.trim().split('\n');
    
    // 行号 1-based → 0-based
    const startIdx = lineInfo.startLine - 1;
    const endIdx = lineInfo.endLine ? lineInfo.endLine - 1 : startIdx + searchLines.length - 1;
    
    // 边界检查
    if (startIdx < 0 || endIdx >= sourceLines.length) return null;
    
    // 验证内容相似度 (≥70%)
    const targetContent = sourceLines.slice(startIdx, endIdx + 1).join('\n').trim();
    const similarity = calculateLineSimilarity(targetContent, searchBlock.trim());
    
    if (similarity >= 0.7) {
        // ✅ 接受定位，执行替换
        const before = sourceLines.slice(0, startIdx);
        const after = sourceLines.slice(endIdx + 1);
        return [...before, replaceBlock.trim(), ...after].join('\n');
    }
    
    return null; // 回退到其他策略
};
```

**优先级:** 行号锚定 > AST Smart Patch > Token 匹配 > 文本回退

---

### Token Matching 多策略瀑布 (核心算法)

**文件位置**: `lib/patch.ts`

**核心函数**: `applyPatchesInternal()`

**🆕 Token Normalization (P2 优化)**

在所有 Token 匹配策略执行前，首先对 Token 进行归一化处理，消除因引号风格、可选分号等导致的虚假不匹配：

```typescript
/**
 * Token Normalization Rules:
 * 1. 引号归一化: " → ', ` → '  (双引号/模板字符串 统一为 单引号)
 * 2. 分号归一化: 在 JS/TS 中分号是可选的，不应导致匹配失败
 * 
 * 示例场景:
 * - AI 输出: const name = "value"
 * - 源码:   const name = 'value'
 * - 归一化后: 两者 Token 序列相同，匹配成功
 */

interface Token {
    text: string;      // 原始文本
    normalized: string; // 🆕 归一化后文本
    start: number;
    end: number;
}

function normalizeToken(text: string): string {
    // Rule 1: 引号归一化
    if (text === '"' || text === '`') return "'";
    return text;
}

// 比较函数优先使用归一化后的值
function tokensEqual(a: Token, b: Token): boolean {
    if (a.text === b.text) return true; // 快速路径
    return a.normalized === b.normalized; // 归一化比较
}
```

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Strategy 0: AST Smart Patch (变量/函数整体替换)                                  │
│  ├── 触发条件: replaceBlock 是完整的 const/function 定义                         │
│  ├── 算法: 解析 replaceBlock AST → 提取变量名 → 在源码中定位同名定义 → 整体替换    │
│  └── 示例: const MONSTERS = [...] 整体替换                                      │
│  ✅ 成功 → 结束 | ❌ 失败 → 继续                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Strategy 1: Exact Token Match (精确 Token 匹配)                                 │
│  ├── 算法:                                                                      │
│  │   1. tokenize(searchBlock) → ['const', 'x', '=', '1', ';']                  │
│  │   2. tokenize(sourceCode) → [...全部 tokens...]                             │
│  │   3. 滑动窗口: 连续匹配所有 search tokens                                     │
│  ├── 特点: 忽略空格/换行，只比较语义单元                                          │
│  └── 示例: "const  x=1" 可以匹配 "const x = 1"                                  │
│  ✅ 成功 → 结束 | ❌ 失败 → 继续                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Strategy 1.5: Comment-Insensitive Match (注释无关匹配) 🆕                       │
│  ├── 触发条件: 精确匹配失败 + searchBlock 包含注释                                │
│  ├── 算法:                                                                      │
│  │   1. stripComments(searchBlock) - 移除所有注释                               │
│  │   2. 用去注释的 tokens 重新进行 Relaxed Match                                │
│  ├── 目的: 解决 AI 幻觉添加注释 (如 {/* Fog of War */}) 的问题                   │
│  └── 示例: AI 输出包含注释，但源码无注释 → 仍然能匹配                              │
│  ✅ 成功 → 结束 | ❌ 失败 → 继续                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Strategy 2: Relaxed Token Match (宽松 Token 匹配)                               │
│  ├── 触发条件: relaxedMode = true (上传的代码首次编辑)                            │
│  ├── 算法: LCS (Longest Common Subsequence)                                     │
│  │   - 允许跳过部分 tokens (容忍轻微差异)                                        │
│  │   - 阈值: 匹配率 > 70%                                                       │
│  └── 示例: 源码多了空行或注释，仍能匹配                                           │
│  ✅ 成功 → 结束 | ❌ 失败 → 继续                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Strategy 3: AST Lite - Function Body Match (函数体匹配)                         │
│  ├── 算法:                                                                      │
│  │   1. 从 searchBlock 提取函数名 (如 handleMove)                               │
│  │   2. 在源码中定位该函数的完整范围 (start-end)                                  │
│  │   3. 整体替换函数体                                                          │
│  ├── 特点: 不需要精确匹配内容，只需函数名匹配                                      │
│  └── 示例: function handleMove() {...} 整体替换                                 │
│  ✅ 成功 → 结束 | ❌ 失败 → 继续                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Strategy 4: Anchor Matching (锚点匹配)                                          │
│  ├── 触发条件: searchBlock 包含压缩标记 或 其他策略全部失败                        │
│  ├── 算法:                                                                      │
│  │   1. 取 searchBlock 首行 + 末行作为"锚点"                                     │
│  │   2. 在源码中查找: 首行出现位置 → 在其后 500 行内查找末行                       │
│  │   3. 用 [首行位置, 末行位置] 定义替换范围                                      │
│  ├── 特点: 中间内容无关，只关心边界                                               │
│  └── 示例: 首行 "const MapScreen = () => {" + 末行 "};" → 定位整个组件           │
│  ✅ 成功 → 结束 | ❌ 失败 → 抛出错误                                             │
└─────────────────────────────────────────────────────────────────────────────────┘
```

---

### Safety Nets (安全网机制)

**Safety Net 1: AST Dedupe (去重)**
```typescript
// 检测并删除重复的顶层定义
// 场景: AI 同时输出了新版本和保留了旧版本
dedupeTopLevelBindings(code);
// 示例: 两个 const MapScreen = ... → 只保留后者
```

**Safety Net 2: Undefined Reference Detection**
```typescript
// 检测删除了定义但仍有引用的情况
detectUndefinedReferences(code);
// 示例: AI 删除了 BattleScene 定义，但 <BattleScene /> 仍存在 → 回滚
```

**Safety Net 3: Incremental Validation**
```typescript
// 每个 Patch 应用后立即校验
validateIncrementalPatch(previousSource, newSource, originalDefinitions, patchIndex);
// 检查:
// - 语法是否合法 (Babel parse)
// - 是否丢失了组件定义
// - 是否产生了孤儿代码 (顶层的 hooks 调用)
```

**Safety Net 4: Target Range Restriction**
```typescript
// 限制 Patch 只能应用到 Intent 指定的文件范围内
allowedRanges = findTargetRanges(source, targets);
// 防止: AI 的 searchBlock 意外匹配到错误的位置
```

**🆕 Safety Net 5: Anchor Uniqueness Check (P1 优化)**
```typescript
// 🆕 P1 优化: 锚点匹配前检查首行是否唯一
const startOccurrences = sourceLines.filter(l => l.trim() === startLine).length;

if (startOccurrences > 1) {
    console.log(`⚠️ Start anchor is not unique (${startOccurrences} occurrences), expanding...`);
    // 扩展锚点: 使用前 2 行组合匹配
    const combinedStart = searchLines[0] + '\n' + searchLines[1];
    // 找到唯一匹配后再继续
}
```

**🆕 Safety Net 6: Transactional Patching (P1 优化)**
```typescript
// 🆕 P1 优化: 事务性 Patch 应用
// 所有 Patch 在 workingSource 副本上执行
// 只有全部成功才返回修改后的代码

let workingSource = source; // 工作副本
const appliedPatches: { index: number; before: string; after: string }[] = [];

// 应用每个 Patch...
appliedPatches.push({ index, before: previousSource, after: workingSource });

// 最终检查
if (failCount > 0 && successCount === 0) {
    throw new Error('无法应用修改'); // 完全失败
}

console.log(`✅ TRANSACTION COMPLETE: ${successCount} patches applied`);
return workingSource;
```

---

### stripComments 实现 (注释剥离)

```typescript
function stripComments(code: string): string {
    return code
        .replace(/\{\/\*[\s\S]*?\*\/\}/g, '') // JSX 注释 {/* ... */} (必须先处理)
        .replace(/\/\*[\s\S]*?\*\//g, '')     // 块注释 /* ... */
        .replace(/\/\/.*/g, '');              // 行注释 // ...
}
```

**为什么 JSX 注释要先处理?**
```jsx
// 错误顺序 (先处理块注释):
{/* comment */}  →  { }  // 留下空大括号，破坏 JSX

// 正确顺序 (先处理 JSX 注释):
{/* comment */}  →  (空)  // 完整删除
```

---

### Patch 失败常见原因

| 错误信息 | 原因 | 解决方案 |
|---------|------|---------|
| `找不到匹配的代码块` | AI 的 SEARCH 块与源码不匹配 | Comment-Insensitive Match (已实现) |
| `Patch 结果过短` | AI 只输出了部分代码 | 长度校验 + 回滚 |
| `重复定义` | AI 没有删除旧代码 | AST Dedupe |
| `未定义引用` | AI 删除了组件但留下了使用 | Undefined Reference Detection |
| `语法错误` | AI 输出了不完整的代码 | validateAndFixSyntax() 自动修复 |
| 🆕 `锚点不唯一` | 多个相同的 useEffect/函数签名 | Anchor Uniqueness Check + 扩展锚点 |

---

## 🆕 优化历史 (Changelog)

### 2025-12-11 P2 优化

**P2 优化 (持续改进)**:
5. **Token Normalization**: Token 归一化提升匹配容错
   - 解决: 引号风格差异 (`"foo"` vs `'foo'`) 导致的虚假不匹配
   - 新函数: `normalizeToken()`, `tokensEqual()`, `tokenTextEqual()`
   - 规则: 双引号/模板字符串 → 单引号归一化
   - 文件: `lib/patch.ts`

### 2025-12-11 P0/P1 优化

**P0 优化 (立即修复)**:
1. **STRICT MODE for RAG**: 当 DeepSeek 返回 targets 时，禁用 "mentioned in prompt" 模糊匹配
   - 解决: "screen" 匹配 "LaunchScreen", "DexScreen" 的噪音问题
   - 文件: `lib/code-rag.ts`

2. **Data Skeletonization**: 纯数据文件不再使用 Brutal Truncate
   - 解决: `VISIBILITY_RADIUS` 等常量被截断丢失值的问题
   - 新函数: `dataSkeletonize()` - 保留所有导出 KEY，只截断长 VALUE
   - 文件: `lib/code-rag.ts`

**P1 优化 (短期)**:
3. **Anchor Uniqueness Check**: 锚点匹配前检查首行唯一性
   - 解决: 多个相同 `useEffect` 导致匹配错位的问题
   - 策略: 首行不唯一时，自动扩展为前 2 行组合匹配
   - 文件: `lib/patch.ts`

4. **Transactional Patching**: 事务性 Patch 应用
   - 解决: 部分 Patch 失败导致代码"半死不活"的问题
   - 策略: 在副本上执行所有 Patch，全部成功才提交
   - 文件: `lib/patch.ts`

---

## 附录: 关键配置

### Babel Parser 配置
```typescript
export const BABEL_PARSER_CONFIG = {
    sourceType: 'module',
    plugins: [
        'jsx', 'typescript', 'decorators-legacy', 
        'classProperties', 'objectRestSpread',
        'optionalChaining', 'nullishCoalescingOperator',
        'dynamicImport', 'exportDefaultFrom'
    ],
    errorRecovery: true
};
```

### Token 化正则 (带归一化)
```typescript
// 基础 Token 提取
const regex = /([a-zA-Z0-9_$]+)|([^\s\w])/g;
// 匹配:
// - 标识符: myVar, useState, $element
// - 符号: {, }, (, ), =, ;, <, >
// 不匹配:
// - 空白字符 (空格, 换行, Tab)

// 🆕 Token 归一化
function normalizeToken(text: string): string {
    if (text === '"' || text === '`') return "'";  // 引号归一化
    return text;
}
```

---

*最后更新: 2025-12-14*

---

## 高级 RAG 优化

> 💡 **另见**: [高级优化路线图](#高级优化路线图-advanced-optimization-roadmap) 了解更多 Token 优化策略

> **文件位置**: [lib/advanced-rag.ts](../lib/advanced-rag.ts)

### 1. 语义缓存 (Semantic Cache)

语义缓存通过缓存相似查询的结果，避免重复调用 LLM 进行意图分类。

**两种实现:**

| 类型 | Text Cache (推荐) | Semantic Cache |
|------|-------------------|----------------|
| **相似度算法** | N-gram Jaccard | 向量余弦相似度 |
| **阈值** | 0.75 (75%) | 0.92 (92%) |
| **需要 Embedding** | ❌ 不需要 | ✅ 需要 |
| **速度** | ⚡ 极快 (<1ms) | 🐢 较慢 (~50ms) |
| **适用场景** | 意图分类缓存 | 复杂语义匹配 |
| **集成位置** | `intent-classifier.ts` | 预留 |

**Text Cache 配置:**
```typescript
const TEXT_CACHE_CONFIG = {
  maxSize: 200,              // 最大条目数
  defaultTTL: 30 * 60 * 1000, // 30 分钟 TTL
  similarityThreshold: 0.75,  // 75% Jaccard 相似度
  ngramSize: 3               // 使用 3-gram
};
```

**工作流程:**
```
用户查询 ──▶ 提取 N-gram ──▶ 遍历缓存 ──▶ Jaccard ≥ 0.75? ──┬──▶ ✅ 返回缓存
                                                            │
                                                            └──▶ ❌ 调用 LLM ──▶ 存入缓存
```

**N-gram Jaccard 相似度算法:**
```typescript
// 提取 N-gram
function extractNgrams(text: string, n: number): Set<string> {
    const normalized = text.toLowerCase().replace(/\s+/g, ' ').trim();
    const ngrams = new Set<string>();
    for (let i = 0; i <= normalized.length - n; i++) {
        ngrams.add(normalized.slice(i, i + n));
    }
    return ngrams;
}

// Jaccard 相似度 = |A ∩ B| / |A ∪ B|
function jaccardSimilarity(a: Set<string>, b: Set<string>): number {
    const intersection = new Set([...a].filter(x => b.has(x)));
    const union = new Set([...a, ...b]);
    return intersection.size / union.size;
}
```

---

### 2. 程序切片 (Program Slicing)

程序切片通过数据流分析，精确提取与目标变量相关的代码，减少 LLM 输入 Token。

**数据流图节点:**
```typescript
interface DataFlowNode {
  name: string;           // 变量/函数名
  type: 'variable' | 'function' | 'parameter' | 'import';
  definedAt: number;      // 定义行号
  usedAt: number[];       // 使用位置列表
  dependsOn: string[];    // 依赖的节点
  dependedBy: string[];   // 被依赖的节点
}
```

**切片方向:**
```typescript
function computeProgramSlice(
  code: string,
  targetName: string,
  direction: 'backward' | 'forward' | 'both'
): ProgramSlice | null;
```

| 方向 | 说明 | 示例 |
|------|------|------|
| **backward** | 所有影响目标的代码 | `gridColor` ← `INITIAL_COLOR`, `colorConfig` |
| **forward** | 所有被目标影响的代码 | `gridColor` → `renderGrid()`, `GridComponent` |
| **both** | 双向切片 (默认) | 完整依赖图 |

**示例:**
```
目标: gridColor

Backward Slice (影响 gridColor 的):
├── INITIAL_COLOR (常量)
└── colorConfig (配置对象)

Forward Slice (被 gridColor 影响的):
├── renderGrid() (使用 gridColor)
└── GridComponent (渲染组件)

压缩比: 原始 2000 行 → 切片 150 行 (92.5% 压缩)
```

---

### 3. 反思代理 (Reflection Agent)

反思代理在补丁应用成功后进行二次检查，确保生成的代码没有语法错误或引用问题。

**检查类型:**
```typescript
interface ReflectionError {
  type: 'syntax' | 'reference' | 'type' | 'logic';
  message: string;
  line?: number;
  column?: number;
  severity: 'error' | 'warning';
}
```

**检查流程:**

```
┌─────────────────────────────────────────────────────────────────────┐
│                      Reflection Agent Pipeline                      │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  1. 语法检查 (checkSyntax)                                          │
│     ├── Babel AST 解析                                              │
│     ├── 捕获 SyntaxError                                            │
│     └── 提取错误位置 (line, column)                                  │
│                         │                                           │
│                         ▼                                           │
│  2. 引用检查 (checkReferences)                                       │
│     ├── 收集所有定义:                                                │
│     │   • VariableDeclarator (变量)                                 │
│     │   • FunctionDeclaration (函数)                                │
│     │   • ImportSpecifier (导入)                                    │
│     ├── 过滤内置全局变量 (window, console, React hooks, etc.)        │
│     └── 检测使用但未定义的标识符                                      │
│                         │                                           │
│                         ▼                                           │
│  3. 生成修复建议                                                     │
│     └── suggestions: ["变量 X 未定义，请确保导入"]                    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

**配置:**
```typescript
const REFLECTION_CONFIG = {
  enableSyntaxCheck: true,      // 启用语法检查
  enableReferenceCheck: true,   // 启用引用检查
  maxErrorsToReport: 10,        // 最多报告 10 个错误
  autoFixAttempts: 2            // 自动修复尝试次数
};
```

**集成位置:** [lib/self-repair.ts](../lib/self-repair.ts)

补丁成功后自动调用 `runReflectionCheck()`：
```typescript
// self-repair.ts
if (patchResult.success) {
    const reflection = runReflectionCheck(patchResult.code);
    if (!reflection.passed) {
        console.warn('[Reflection] Issues found:', reflection.errors);
        // 触发修复流程...
    }
}
```

---

## 自愈循环系统

> **文件位置**: [lib/self-repair.ts](../lib/self-repair.ts)

### 工作流程

```
┌─────────────────────────────────────────────────────────────────────┐
│                      Self-Repair Loop (自愈循环)                    │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  1. 尝试应用补丁 (applyPatchesWithDetails)                           │
│            │                                                        │
│            ├── 成功 ─────────────────┐                              │
│            │                         ▼                              │
│            │              ┌─────────────────────┐                   │
│            │              │ Reflection Check    │                   │
│            │              │ 语法 + 引用检查      │                   │
│            │              └─────────────────────┘                   │
│            │                         │                              │
│            │              ┌──────────┴──────────┐                   │
│            │              ▼                     ▼                   │
│            │        ┌─────────┐           ┌─────────┐               │
│            │        │ 通过    │           │ 失败    │               │
│            │        └─────────┘           └─────────┘               │
│            │              │                     │                   │
│            │              ▼                     │                   │
│            │        ┌─────────┐                 │                   │
│            │        │ ✅ 完成  │                 │                   │
│            │        └─────────┘                 │                   │
│            │                                    │                   │
│            ├── 失败 ◄───────────────────────────┘                   │
│            │                                                        │
│            ▼                                                        │
│  2. 分析失败原因 (analyzePatchFailure)                               │
│     ├── search_mismatch: SEARCH 块找不到匹配                         │
│     ├── syntax_error: 补丁后语法错误                                  │
│     └── reference_broken: 引用完整性破坏                              │
│                                                                     │
│  3. 构建修复提示词 (buildRepairPrompt)                               │
│     ├── 原始代码                                                     │
│     ├── 失败的补丁                                                   │
│     ├── 错误信息                                                     │
│     └── 匹配上下文 (最相似的代码段)                                   │
│                                                                     │
│  4. 请求 LLM 重新生成                                                │
│            │                                                        │
│            ▼                                                        │
│  5. 重试应用 (attempt < maxRetries)                                  │
│     ├── maxRetries = 2                                              │
│     ├── 成功 → 返回结果                                              │
│     └── 失败 → 继续循环或放弃                                        │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 配置项

```typescript
interface SelfRepairConfig {
  maxRetries: number;           // 最大重试次数 (默认 2)
  enableSyntaxCheck: boolean;   // 启用语法检查
  enableReferenceCheck: boolean; // 检查引用完整性
  logLevel: 'silent' | 'normal' | 'verbose';
}
```

---

## 文件映射与导出函数

### 核心文件

| 文件路径 | 功能 | 行数 |
|---------|------|------|
| [lib/intent-classifier.ts](../lib/intent-classifier.ts) | 意图分类 + Text Cache 集成 | ~1300 |
| [lib/code-rag.ts](../lib/code-rag.ts) | RAG 检索 + GraphRAG + 语义压缩 | ~3200 |
| [lib/patch.ts](../lib/patch.ts) | 补丁应用 + 安全网 + 行号锚定 | ~2200 |
| [lib/self-repair.ts](../lib/self-repair.ts) | 自愈循环 + Reflection 集成 | ~600 |
| [lib/advanced-rag.ts](../lib/advanced-rag.ts) | 高级优化 (缓存/切片/反思) | ~1000 |
| [lib/ast-parser.ts](../lib/ast-parser.ts) | AST 分析工具 | ~500 |

### 导出函数速查

```typescript
// intent-classifier.ts
export { classifyUserIntent, UserIntent, SearchStrategy };

// code-rag.ts
export { 
  chunkCode,                    // 代码分块
  findRelevantCodeChunks,       // RAG 检索
  semanticPrecompress,          // P3: 语义压缩
  buildDependencyGraph,         // P2: 构建依赖图
  expandContextByGraph,         // P2: GraphRAG 扩展
  BABEL_PARSER_CONFIG           // Babel 配置共享
};

// patch.ts
export { 
  applyPatches,                 // 补丁应用 (返回代码)
  applyPatchesWithDetails,      // 补丁应用 (返回详情)
  PatchResult, 
  PatchStats 
};

// self-repair.ts
export { 
  analyzePatchFailure,          // 分析失败原因
  buildRepairPrompt,            // 构建修复提示词
  RepairResult 
};

// advanced-rag.ts
export {
  // Text Cache (推荐)
  queryTextCache,               // 查询文本缓存
  storeTextCache,               // 存储到文本缓存
  
  // Semantic Cache
  querySemanticCache,           // 查询语义缓存
  storeSemanticCache,           // 存储到语义缓存
  
  // Program Slicing
  computeProgramSlice,          // 计算程序切片
  buildDataFlowGraph,           // 构建数据流图
  
  // Reflection Agent
  runReflectionCheck,           // 运行反思检查
  runReflectionWithAutoFix,     // 带自动修复的反思检查
  generateFixPrompt             // 生成修复提示词
};
```

---

## 性能指标

| 指标 | 目标值 | 说明 |
|------|--------|------|
| 意图分类延迟 | < 100ms (本地) / < 500ms (DeepSeek) | 本地关键词优先 |
| Text Cache 命中 | < 1ms | N-gram 比较 |
| RAG 检索延迟 | < 300ms | 包含向量搜索 |
| 补丁应用成功率 | > 85% | 首次尝试 |
| 自愈后成功率 | > 95% | 包含重试 |
| Text Cache 命中率 | > 40% | 重复/相似查询 |

---

## 优化历史 (Changelog)

### 2025-12-14 ✅ P0-P4 优化全部实施完成
**实施完成**: P0 Prompt Caching + P1 Type Definitions + P3 PageRank + P4 Unified Diff

**P0: Prompt Caching (三级缓存系统)**
- 实现文件: `lib/prompt-cache.ts` (626 行)
- L1: 系统提示词缓存 (跨用户共享，永不过期)
- L2: 项目骨架缓存 (10 分钟 TTL)
- L3: 会话上下文缓存 (5 分钟 TTL)
- 支持 DeepSeek + Gemini 隐式缓存优化

**P1: Type Definitions Generator (类型定义生成)**
- 实现位置: `lib/advanced-rag.ts` - `generateTypeDefinition()`
- 集成位置: `lib/code-rag.ts` - `applyTrustModeCompression()`
- 功能: 将参考文件 (files_to_read) 转换为 .d.ts 风格类型定义
- 压缩效果: 30-50% Token 节省
- 智能 fallback: 类型定义无效时回退到骨架压缩

**P3: GraphRAG PageRank Pruning (依赖图剪枝)**
- 实现位置: `lib/advanced-rag.ts` - `computePageRank()`, `pruneGraphByPageRank()`
- 集成位置: `lib/code-rag.ts` - GraphRAG 扩展流程
- 功能: 对 >15 节点的依赖图使用 PageRank 算法智能剪枝
- 配置: 阻尼系数 0.85，保留 top 70%，目标节点自动 boost
- 效果: 防止复杂项目上下文爆炸

**P4: Unified Diff 支持**
- 实现位置: `lib/advanced-rag.ts` - `generateUnifiedDiff()`, `applyUnifiedDiff()`
- 辅助函数: `isDiffOutput()`, `buildDiffOutputInstructions()`, `selectOutputFormat()`
- 功能: 生成和应用 unified diff 格式，减少 30% 输出 Token
- 算法: 基于 LCS (最长公共子序列) 的 diff 计算

### 2025-12-19 🚀 高级优化路线图 (P0-P4)
**新增章节**: [高级优化路线图](#高级优化路线图-advanced-optimization-roadmap)
- P0: Prompt Caching (API级缓存) - 预期成本降低 90%
- P1: 类型定义 (.d.ts) 代替代码骨架 - 预期 Token 减少 30%
- P3: GraphRAG 剪枝 (PageRank) - 防止上下文爆炸
- P4: Unified Diff 输出格式 - 输出 Token 减少 30%
- 实施路线图：Phase 1 (1-2周) → Phase 2 (3-4周)

### 2025-12-18 自适应压缩策略
- 新增 500 行阈值决策树
- < 500 行：完整代码 + 依赖图
- ≥ 500 行：语义切片 + 类型定义
- 添加场景对比表格

### 2025-01-15 高级 RAG 优化

**P1: 语义缓存 (Semantic Cache)**
- 新增 Text Cache (N-gram Jaccard)，集成到 `intent-classifier.ts`
- 新增 Semantic Cache (向量相似度)，预留在 `advanced-rag.ts`
- 预期效果：减少 40%+ 的 DeepSeek API 调用

**P2: 程序切片 (Program Slicing)**
- 新增 `buildDataFlowGraph()` 数据流图构建
- 新增 `computeProgramSlice()` 双向切片计算
- 预期效果：精确提取目标相关代码，减少 60%+ Token

**P3: 反思代理 (Reflection Agent)**
- 新增 `runReflectionCheck()` 语法/引用检查
- 集成到 `self-repair.ts` 补丁成功后调用
- 预期效果：减少语法错误和引用问题

### 2025-01-14 Patch 系统优化

**P1: 行号锚定 (Line Anchoring)**
- 补丁格式支持 `@L42-L58` 行号范围
- 优先使用行号定位，相似度 ≥ 70% 接受
- 文件: `lib/patch.ts`

**P2: 函数级分块 (Function-Level Chunking)**
- AST 解析按函数边界分块
- 保留完整函数上下文
- 文件: `lib/code-rag.ts`

**P2: GraphRAG 增强**
- 依赖图构建 `buildDependencyGraph()`
- 智能上下文扩展 `expandContextByGraph()`
- 向下扩展依赖 + 向上扩展被依赖
- 文件: `lib/code-rag.ts`

**P3: 语义压缩预处理**
- `semanticPrecompress()` 删除注释/空行/console.log
- 截断超长字符串
- 节省 15-25% Token
- 文件: `lib/code-rag.ts`

### 2025-12-11 P0/P1/P2 优化

**P0: STRICT MODE for RAG**
- 当 DeepSeek 返回 targets 时，禁用 "mentioned in prompt" 模糊匹配
- 解决: "screen" 匹配 "LaunchScreen" 的噪音问题
- 文件: `lib/code-rag.ts`

**P0: Data Skeletonization**
- 纯数据文件不再使用 Brutal Truncate
- 保留所有导出 KEY，只截断长 VALUE
- 文件: `lib/code-rag.ts`

**P1: Anchor Uniqueness Check**
- 锚点匹配前检查首行唯一性
- 首行不唯一时，自动扩展为前 2 行组合匹配
- 文件: `lib/patch.ts`

**P1: Transactional Patching**
- 事务性 Patch 应用
- 在副本上执行所有 Patch，全部成功才提交
- 文件: `lib/patch.ts`

**P2: Token Normalization**
- 引号归一化 (`"` → `'`, `` ` `` → `'`)
- 解决引号风格差异导致的虚假不匹配
- 文件: `lib/patch.ts`

---

*最后更新: 2025-01-15*
